...
const string REAL_FILE[] = {
                           "REALGRAPHS\\artist_edges.txt"
                           };
...
int main() {
...
#ifdef REAL_GRAPH
    vector<pair<int, int>> e = readFromFileNM(REAL_FILE[TEST], n, m);
#endif
	...
	//alg.AingworthAlgorithm();
	...
	#endif
    #ifdef REAL_GRAPH
        const int REAL_R = RADIUS_REAL[TEST];
    #endif
...
#ifdef OUR_GA
		{
			...
			for (int i = 0; i < ITER; i++) {
				...
				GeneticAlgorithm genAlg(g1, 20, 0.7, 0.1);
				double time = 0.0;
